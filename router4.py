...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    sock_router2 = create_socket("localhost", 8002)
    sock_router5 = create_socket("localhost", 8005)
    sock_router6 = create_socket("localhost", 8006)
    ...
    while True:
        ...
        for entry in forwarding_table_with_range:
            ip_min, ip_max = entry[4]
            if ip_min <= destinationIP_int <= ip_max:
                send_to_router = entry[3]
                break
